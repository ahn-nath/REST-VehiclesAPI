...
public class CarControllerTest {
	...
	Car carUpdated = updateCar(newModel);
		
	 mvc.perform(put(new URI("/cars/1")) .accept(MediaType.APPLICATION_JSON_UTF8)
			  .contentType(MediaType.APPLICATION_JSON_UTF8)
			  .content(json.write(carUpdated).getJson())) .andExpect(status().isOk())
			  .andExpect(jsonPath("$.details.model").value(newModel));	
		
	}
	
	private Car updateCar(String newModel) {
